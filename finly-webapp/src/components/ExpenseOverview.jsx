...
const ExpenseOverview = () => {
  ...
  const fetchExpenseData = async () => {
    try {
      setError(null);
      const response = await axiosConfig.get(`${API_ENDPOINTS.EXPENSE}/current-month`);
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-400">
            Track your expense transactions and analyze your spending
          </p>
        </div>
        {chartData.length === 0 ? (
          <div className="h-[300px] w-full flex items-center justify-center bg-gray-50 rounded-lg">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400">Add your first expense to see the chart</p>
            </div>
          </div>
        ) : (
          <div className="h-[300px] w-full">
            <CustomLineChart data={chartData} isExpense={true} isIncome={false} />
          </div>
        )}
      </div>
    </div>
